...
impl EmbeddingEngine {
    pub fn new() -> Result<Self> {
        ...
        // 🚨 TITANIUM FIX: Khai báo lỗi Tokenizers
        let mut tokenizer = Tokenizer::from_file(&tokenizer_path)
            .map_err(|e: Box<dyn std::error::Error + Send + Sync>| OuroborosError::Reasoning(format!("Lỗi Tokenizer: {}", e)))?;
        ...
        let config_str = std::fs::read_to_string(&config_path)
            .map_err(|e: std::io::Error| OuroborosError::Reasoning(e.to_string()))?;
        let mut config: BertConfig = serde_json::from_str(&config_str)
            .map_err(|e: serde_json::Error| OuroborosError::Reasoning(e.to_string()))?;
        
        config.hidden_act = HiddenAct::Gelu;

        let vb = unsafe {
            VarBuilder::from_mmaped_safetensors(&[&weights_path], candle_core::DType::F32, &device)
                .map_err(|e: candle_core::Error| OuroborosError::Reasoning(format!("Lỗi Mmap: {}", e)))?
        };

        let model = BertModel::load(vb, &config)
            .map_err(|e: candle_core::Error| OuroborosError::Reasoning(format!("Lỗi tải Model: {}", e)))?;

        info!("✅ [Embedding Engine] Đã sẵn sàng nén tri thức thành Vector 384 chiều.");

        Ok(Self { model, tokenizer, device, dimension: 384 })
    }
    ...
    pub fn generate_embedding(&self, text: &str) -> Result<Vec<f32>> {
        ...
        let embeddings = self.model.forward(&token_tensor, &token_type_ids, None)
            .map_err(|e: candle_core::Error| OuroborosError::Reasoning(format!("Lỗi Forward BERT: {}", e)))?;
    ...
    fn download_minilm_core() -> Result<(std::path::PathBuf, std::path::PathBuf, std::path::PathBuf)> {
        ...
        if !cache_dir.exists() {
            std::fs::create_dir_all(&cache_dir)
                .map_err(|e: std::io::Error| OuroborosError::Reasoning(format!("Lỗi tạo thư mục: {}", e)))?;
        }
        ...
        for file in files {
            ...
            let status = std::process::Command::new("curl")
                ...
                .status()
                .map_err(|e: std::io::Error| OuroborosError::Reasoning(format!("OS từ chối lệnh cURL: {}", e)))?;
